	...
	for topic_id, topic_name, domain_id in topics:
		...
		fh.write(','.join(tmplist) + '\r\n')
            for subtopic_id, subtopic_name in subtopics:
                atn_db.cur.execute('SELECT passage_id, passage_name, docno, offset_start, offset_end, grade, create_time FROM passage WHERE subtopic_id=? AND state !=-1 AND state !=2', [subtopic_id,])
